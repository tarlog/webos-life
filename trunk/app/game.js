...
var game = {
	...
	gameConsole : {
		...
		leftHandLast : null,
		rightHandLast : null,
		leftHandCurrrentState : null,
		rightHandCurrentState : null,
		lightsDelay : null,
		lightsLeft : [ {
			y400 : 28,
			y480 : 28,
	...
	init : function() {
		...
		// Bind method for listening for orientation change events and stage
		// events.
		...
		// this.gameConsole.right = new Image();s
		...
		// this.gameConsole.lightsLeft[i].img.src = "images/frame-l-" + i
		// + ".png";
		// this.gameConsole.lightsRight[i].img = new Image();
		// this.gameConsole.lightsRight[i].img.src = "images/frame-r-" + i
		// + ".png";
		// }
		// for (i = 0; i < 4; i++) {
	...
	startNewGame : function() {
		...
		// // Start chicken off in the middle and on the first frame of
		// animation.
		// this.chicken.x = 128;
		...
		// this.chicken.dropX = Math.floor(Math.random() * (200 - this.xAdj)) +
		// 28;
		...
		// this.ctx.drawImage(this.gameConsole.middle, 144,
		// this.screenHeight - 80, 49,
		// this.gameConsole.consoleBottomHeight);
		// this.ctx.drawImage(this.gameConsole.right, 287, this.screenHeight -
		// 80,
	...
	mainLoop : function() {
		...
		if (this.gameConsole.lightsDelay > 11) {
			...
			for (i = 0; i < 5; i++) {
				...
				if (n == 1) {
					this.ctx.drawImage(this.gameConsole.lightsLeft[i].img, 0,
							this.gameConsole.lightsLeft[i][propName], 27,
							this.gameConsole.lightsLeft[i].height);
				}
				n = Math.floor(Math.random() * 2);
				if (n == 1) {
					this.ctx.drawImage(this.gameConsole.lightsRight[i].img,
							293, this.gameConsole.lightsRight[i][propName], 27,
							this.gameConsole.lightsRight[i].height);
				}
			}
		}
		...
		for (i = 0; i < this.eggData.count; i++) {
			...
			if (this.eggData.eggs[i].y > 900) {
				this.ctx
						.drawImage(this.eggData.eggSplat, this.xAdj
								+ this.eggData.eggs[i].x - 14, this.yAdj
								+ (this.screenHeight - 150),
								this.eggData.eggSplatWidth,
								this.eggData.eggSplatHeight);
				...
				if (this.eggData.eggs[i].y < 980) {
					...
					if (this.catcher.damage > 3) {
						...
						this.dialog = this.mainAssistant.controller
								.showDialog( {
				...
				this.ctx.drawImage(this.eggData.egg, this.xAdj
						+ this.eggData.eggs[i].x, this.yAdj
						+ this.eggData.eggs[i].y, this.eggData.eggWidth,
						this.eggData.eggHeight);
			}

			// Check egg for collision with catcher. If caught, increase score,
			// and
			// speed if it's time.
			if (this.eggData.eggs[i].x >= this.catcher.collisionBox.x1
				...
				if (this.score > (this.previousScore + this.scoreFlipAmount)) {
					...
					// Increment the speed flip counter so the speed increases
					// slower
					// as we go.
			...
			if (!(this.eggData.eggs[i].x == 0 && this.eggData.eggs[i].y == 0)
				...
				this.eggData.eggs[i].y = this.eggData.eggs[i].y
						+ this.chicken.speed;
				// If the egg reaches the bottom, set the coordinates to 1000 so
				// we know
				// we have to display it splattered for a few frames before
				// clearing it.
		...
		this.ctx.drawImage(this.chicken.imgs[this.chicken.animationFrame],
				this.xAdj + this.chicken.x, this.yAdj + 2, this.chicken.width,
				this.chicken.height);
		...
		if (!this.explosion.exploding) {
			...
			if (dropEgg) {
				...
				// Add a new egg descriptor object to the array of eggs
				// dropping.
		...
		this.ctx.drawImage(this.catcher.imgs[this.catcher.damage], this.xAdj
				+ this.catcher.x, this.yAdj + (this.screenHeight - 156),
				this.catcher.width, this.catcher.height);
		// Moving left.
		if (this.gameConsole.leftHandCurrentState == this.gameConsole.LEFT_HAND_LEFT) {
			...
			// Moving right.
		} else if (this.gameConsole.leftHandCurrentState == this.gameConsole.LEFT_HAND_RIGHT) {
			this.catcher.x = this.catcher.x + 4;
			// Stop at edge of screen.
		...
		if (this.gameConsole.leftHandLast != this.gameConsole.leftHandCurrentState
		...
		if (this.explosion.exploding) {
			// Deaw it.
			this.ctx.drawImage(
					this.explosion.imgs[this.explosion.animationFrame],
	...
	drawHands : function() {
		...
		this.ctx
				.drawImage(
						...
						39, this.screenHeight - 80,
						this.gameConsole.leftHandWidth,
						this.gameConsole.consoleBottomHeight);
		this.ctx
				.drawImage(
						this.gameConsole.rightHand[this.gameConsole.rightHandCurrentState],
						193, this.screenHeight - 80,
						this.gameConsole.rightHandWidth,
						this.gameConsole.consoleBottomHeight);
	...
	keyDown : function(inEvent) {
		...
		if (this.gameInProgress) {
			...
			case Mojo.Char.q:
			case Mojo.Char.q + 32:
				this.gameConsole.leftHandCurrentState = this.gameConsole.LEFT_HAND_LEFT;
				break;

			// Move right.
			case Mojo.Char.p:
			case Mojo.Char.p + 32:
				this.gameConsole.leftHandCurrentState = this.gameConsole.LEFT_HAND_RIGHT;
				break;
			...
			case Mojo.Char.spaceBar:
				if (!this.missile.inFlight) {
					...
					this.gameConsole.rightHandCurrentState = this.gameConsole.RIGHT_HAND_DOWN;
				}
				break;
	...
	tapHandler : function(inEvent) {
		...
		xcell = Math.floor(x / 11);
		ycell = Math.floor(y / 11);

		this.ctx.drawImage(this.liveCell, this.xAdj + xcell * 11, this.yAdj
				+ ycell * 11);
